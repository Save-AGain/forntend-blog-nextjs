...
export default function BlogPage() {
  ...
  const [currentPage, setCurrentPage] = useState(1); // กำหนดค่าเริ่มต้นเป็น 1
  const [blogs, setBlogs] = useState<Blog[]>([]);
  const [totalPages, setTotalPages] = useState(1);

  // ดึงค่าหน้าเว็บจาก URL
  useEffect(() => {
    const page = new URLSearchParams(window.location.search).get("page");
    setCurrentPage(page ? Number(page) : 1); // ถ้ามีค่าจาก query string ให้ใช้
  }, []);

  // ฟังก์ชันดึงข้อมูล blog
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_STRAPI_BASE_URL}/api/blogs?pagination[page]=${currentPage}&pagination[pageSize]=${PAGE_SIZE}&populate[0]=tags&populate[1]=Images_cover&sort[0]=createdAt:desc`
      );
  ...
  const handlePageChange = (newPage: number) => {
    ...
    setCurrentPage(newPage); // อัปเดตค่าหน้าเว็บหลังจากการคลิก
  };
